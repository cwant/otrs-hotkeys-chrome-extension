...
$('[data-trigger="floater"]').removeClass('FloaterOpen');$TriggerObj.addClass('FloaterOpen');$FloaterObj=$('#AppWrapper > .MetaFloater.Hidden').clone();$('body > div.MetaFloater:visible').remove();$FloaterObj.removeClass('Hidden').appendTo('body').css('display','none');FloaterWidth=parseInt($FloaterObj.outerWidth(),10);if(TriggerOffset.left<FloaterWidth/2){$FloaterObj.addClass('Left').css({right:'auto',left:TriggerOffset.left});}
else if(ViewportWidth-TriggerOffset.left+($TriggerObj.outerWidth()/2)<FloaterWidth/2){$FloaterObj.addClass('Right').css({right:'auto',left:TriggerOffset.left-FloaterWidth+$TriggerObj.outerWidth()});}
...
"use strict";var Core=Core||{};Core.UI=Core.UI||{};Core.UI.Table=(function(TargetNS){TargetNS.InitTableFilter=function($FilterInput,$Container,ColumnNumber){var Timeout,$Rows=$Container.find('tbody tr:not(.FilterMessage), li:not(.Header):not(.FilterMessage)'),$Elements=$Rows.closest('tr, li');if(typeof ColumnNumber==='string'||typeof ColumnNumber==='number'){$Rows=$Rows.find('td:eq('+ColumnNumber+')');}
...
.addClass('IsResized');}
$('#Navigation > li').each(function(){NavigationBarWidth+=parseInt($(this).outerWidth(true),10);});$('#Navigation').css('width',(NavigationBarWidth+3)+'px');if(NavigationBarWidth>$('#NavigationContainer').outerWidth()){NavigationBarShowSlideButton('Right',parseInt($('#NavigationContainer').outerWidth(true)-NavigationBarWidth,10));}
...
else{CheckSearchStringsForStopWords(function(){$('#SearchForm').submit();ShowWaitingDialog();});}}
